...
All example described below can be done using the Docker image by executing in the directory containing your templates `docker run -v $(pwd):/data mcorbin/ymlgen:v0.1.0 <command>`. The files will be availables in `/data`. Example: `docker run -v $(pwd):/data mcorbin/ymlgen:v0.2.0 yaml -t /data/example.edn`.
